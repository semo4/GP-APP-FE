 ...
 useEffect(() => {
  ...
  const wrappedNewChat = () => {
   ...
   const exportDocx = async () => {
    ...
    // Save to Firestore, etc, if needed...
    // then generate a .docx
    const doc = new Document({
      sections: [{
        properties: {},
        children: [
...
const insertImage = (url) => {
  setImagesMap(prev => {
    const currentImages = prev[articleContent] || [];
    return {
      ...prev,
      [articleContent]: [...currentImages, { url, position: "right" }]
    };
  });
};
    ...
    <div className="generated-articleH">
      ...
      <p className="article-timestampH">{selectedChat?.timestamp}</p>
        ...
        <>
          ...
          {(imagesMap[articleContent] || []).map((imgObj, index) => (
